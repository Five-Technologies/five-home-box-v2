...
void onNotification(Notification const *notification, void *context)
{
	...
	case Notification::Type_ValueRemoved:
		log += "[VALUE_REMOVED]                   node " + to_string(notification->GetNodeId()) + " value " + to_string(valueID.GetId()) + '\n';
	...
	case Notification::Type_ValueChanged:
		...
		log += "[VALUE_CHANGED]                   node " + to_string(valueID.GetNodeId()) + ", " + Manager::Get()->GetValueLabel(valueID) + ": " + *ptr_container + '\n';

		valueID = notification->GetValueID();
		n = getNode(valueID.GetNodeId(), nodes);
		Manager::Get()->GetNodeNeighbors(homeID, valueID.GetNodeId(), n->m_neighbors);
		break;
	case Notification::Type_ValueRefreshed:
		notifType = "VALUE REFRESHED";
		Manager::Get()->GetValueAsString(valueID, ptr_container);
		log += "[VALUE_REFRESHED]                 node " + to_string(valueID.GetNodeId()) + ", " + Manager::Get()->GetValueLabel(valueID) + ": " + *ptr_container + "\n";
		break;
